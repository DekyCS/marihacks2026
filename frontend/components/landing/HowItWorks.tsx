  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', padding: 40 }}>
      ...
      >
        ...
        >
          <div style={{ width: '64%', height: '100%', background: 'var(--pop)' }} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      {['M-101', 'M-112', 'B-200', 'W-002', 'S-301', 'P-017'].map((p, i) => (
        ...
        >
          <svg width={52} height={52} viewBox="0 0 52 52" style={{ color: 'var(--ink)' }}>
            <g transform="translate(26 26)">
              <polygon
                points={i % 2 ? '-16,-10 16,-10 16,10 -16,10' : '-14,-14 14,0 14,14 -14,14 -14,-14'}
                fill="var(--paper-2)"
                stroke="currentColor"
                strokeWidth={1.4}
              />
...
function VoiceBars() {
  return (
    <div style={{ display: 'flex', alignItems: 'end', gap: 3, marginTop: 10, height: 18 }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mono" style={{ position: 'absolute', left: 12, top: 12, color: 'var(--ink-mute)' }}>
          VIEWPORT
        </div>
        <div
          className="mono"
          style={{ position: 'absolute', right: 12, top: 12, color: 'var(--pop)' }}
        >
          STEP 3 / 7
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        >
          <div className="mono" style={{ color: 'var(--pop)', marginBottom: 6 }}>
            ◉ NARRATOR
          </div>
          <div style={{ fontSize: 13, color: 'var(--ink-soft)' }}>
            &ldquo;Take the M-112 bracket and align it with the two pre-drilled holes on the side panel.&rdquo;
          </div>
          <VoiceBars />
        </div>
        <div
        ...
        >
          ...
          <div style={{ fontSize: 13, color: 'var(--ink)' }} className="serif-ital">
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="mono" style={{ color: 'var(--ink-mute)' }}>
          manualy.app / demo / step {active + 1}
        </span>
...
export default function HowItWorks() {
  const ref = useReveal();
  const steps = [
    { n: '01', k: 'Drop your PDF', v: 'Any manual. IKEA, a drone, a crib.', side: 'UPLOAD' },
    {
      n: '02',
      k: 'Rebuilt in 3D',
  ...
  return (
    <section
      id="how"
      style={{ padding: 'var(--pad-y) 0', borderTop: '1px solid var(--rule-strong)' }}
    >
            ...
            {steps.map((s, i) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 18 }}>
                  <span
                    className="mono"
                    style={{ color: active === i ? 'var(--pop)' : 'var(--ink-mute)' }}
                  >
                    {s.n}
                  </span>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ marginBottom: 8 }}>{s.k}</h3>
                    <p style={{ margin: 0, color: 'var(--ink-soft)', fontSize: 16, maxWidth: 460 }}>
                      {s.v}
                    </p>
                  </div>
                  <span className="mono" style={{ color: 'var(--ink-mute)' }}>
